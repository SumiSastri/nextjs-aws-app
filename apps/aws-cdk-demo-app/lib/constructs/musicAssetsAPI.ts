...
export class MusicAssetsAPI extends Construct {
  ...
  constructor(scope: Construct, id: string, props: MusicAssetsAPIProps) {
    ...
    // GET REQUEST
    const getMusicAssetsFunction = new lambda.NodejsFunction(
      this,
      "GetMusicAssetsFunction",
      {
    ...
    bucketPermissions.addResources(
      `${props.musicAssetsBucketProps.bucketArn}/*`
    );
    ...
    const integration = new apiIntegration.HttpLambdaIntegration(
      "MusicAssetsAPIIntegration",
      getMusicAssetsFunction
    );
